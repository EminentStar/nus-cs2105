...
When there is a frame to send:
	\begin{algorithmic}
		...
		\State start sending
		\While{has more bits}
		\State  continue sending
		\EndWhile
		\State stop sending
		\Until frame is sent without collision
	\end{algorithmic}
\end{frame}
...
\begin{frame}
	\tiny
When there is a frame to send, set $n$ = 1
	\begin{algorithmic}
		\Repeat
		\While{channel is busy}
		\State wait
		\EndWhile
		\State start sending
		\While{no collision or has more bits}
		\State  continue sending
		\EndWhile
		\State stop sending 
		\If{there is a collision}
		\State K = random(0,$2^n-1$)
		\State Wait 512K bit time
		\State n = min(n + 1, 10)
		\EndIf
		\Until{frame is sent completely}
	\end{algorithmic}
\end{frame}
...
\begin{cf}{
	...
	\small
	\begin{itemize}
		\item Use FDM to divide upstream and downstream into multiple channels
		\item Use TDM for upstream (channels are shared)
		\item CMTS assign slots to cable modems
		\item Cable modems send slot requests in random access manner.
	\end{itemize}
}
\end{cf}
